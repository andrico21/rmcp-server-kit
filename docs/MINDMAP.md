...
| Server entry (stdio, no auth)     | `src/transport.rs`                       | `serve_stdio` ~L3718                                                          |
...
| Origin / security headers (defs)  | `src/transport.rs`                       | `origin_check_middleware` ~L3625, `security_headers_middleware` ~L3132       |
